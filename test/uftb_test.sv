...
module uftb_test;
    ...
    task chk(
        ...
        logic match_br1;
        match_br0 =
            (fb.br_slot[0].vld == exp_fb.br_slot[0].vld)
        &&  (!fb.br_slot[0].vld || // dont care about mismatch if invalid
                (fb.br_slot[0] == exp_fb.br_slot[0])
            );

        match_br1 =
            (fb.br_slot[1].vld == exp_fb.br_slot[1].vld)
        &&  (!fb.br_slot[1].vld ||
                ((fb.br_slot[1] == exp_fb.br_slot[1]) && (fb.md1 == exp_fb.md1))
            );

        if ((spill != exp_spill) || !match_br0 || !match_br1) begin
