...
module map_table_testbench;
    ...
    initial begin
        ...
        // Print debug info
        $display("DEBUG: Test 1 - Expected rs_out.t1s[0] != 0");
        $display("       src1: %0d -> mapped to t1: %0d", d_in.src1s[0], rs_out.t1s[0]);
        $display("       src2: %0d -> mapped to t2: %0d", d_in.src2s[0], rs_out.t2s[0]);
        $display("       dst: %0d -> mapped to new ts: %0d", d_in.dsts[0], d_in.ts[0]);

        if (rs_out.t1s[0] == 1 && rs_out.t2s[0] == 2) $display("✅ Test 1 Passed: Register renamed!");
        else begin
            $error("❌ Test 1 Failed! Expected rs_out.t1s[0] != 0, but got: %0d", rs_out.t1s[0]);
        end
        ...
        // Print debug info
        $display("DEBUG: Test 2 - Expected rs_out.cpl1s[0] == 1");
        $display("       Completed physical reg: %0d", c_in.c_ts[0]);
        $display("       Completion status (cpl1s[0]): %0d", rs_out.cpl1s[0]);

        if (rs_out.cpl1s[0] == 1) $display("✅ Test 2 Passed: Register completion correct!");
        else begin
            $error("❌ Test 2 Failed! Expected rs_out.cpl1s[0] == 1, but got: %0d", rs_out.cpl1s[0]);
        end
        ...
        // Print debug info
        $display("DEBUG: Test 3 - Expected rs_out.t1s[0] == 0");
        $display("       Zero register mapping: %0d", rs_out.t1s[0]);

        if (rs_out.t1s[0] == 0) $display("✅ Test 3 Passed: ZERO_REG handling correct!");
        else begin
            $error("❌ Test 3 Failed! Expected rs_out.t1s[0] == 0, but got: %0d", rs_out.t1s[0]);
        end
        ...
        $display("       src1[0]: %0d -> mapped to t1: %0d", d_in.src1s[0], rs_out.t1s[0]);
        $display("       src2[0]: %0d -> mapped to t2: %0d", d_in.src2s[0], rs_out.t2s[0]);
        $display("       dst[0]: %0d -> new mapping: %0d", d_in.dsts[0], d_in.ts[0]);

        $display("       src1[1]: %0d -> mapped to t1: %0d (should be 20)", d_in.src1s[1], rs_out.t1s[1]);
        $display("       src2[1]: %0d -> mapped to t2: %0d", d_in.src2s[1], rs_out.t2s[1]);
        $display("       dst[1]: %0d -> new mapping: %0d", d_in.dsts[1], d_in.ts[1]);

        if (rs_out.t1s[0] == 20) $display("✅ Test 4 Passed: Multi-dispatch renaming correct!");
        else $error("❌ Test 4 Failed! Expected rs_out.t1s[1] == 20, but got: %0d", rs_out.t1s[0]);
        ...
        $display("       src1: %0d -> mapped to t1: %0d (should be 31)", d_in.src1s[0], rs_out.t1s[0]);

        if (rs_out.t1s[0] == 31) $display("✅ Test 5 Passed: Register overwrite handled correctly!");
        else $error("❌ Test 5 Failed! Expected rs_out.t1s[0] == 31, but got: %0d", rs_out.t1s[0]);
        ...
        $display("       Completed physical reg: %0d -> cpl1s: %0d", c_in.c_ts[0], rs_out.cpl1s[0]);
        $display("       Completed physical reg: %0d -> cpl1s: %0d", c_in.c_ts[1], rs_out.cpl1s[1]);

        if (rs_out.cpl1s[0] && rs_out.cpl1s[1]) $display("✅ Test 6 Passed: Multi-completion correct!");
