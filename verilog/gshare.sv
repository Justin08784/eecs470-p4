...
`ifdef DEBUG
    ...
    task print_gshare;
        $display(">> gshare");
        if (i_uen)
            $display("i_uhash: (%3h)%b, take: %b, slot_idx: %b, urec: [%b, %b]",
                i_uhash,
                i_uhash,
                i_utake,
                i_uslot_idx,
        ...
        for (int i = 0; i < PHT_SZ; ++i) begin
            if (|touched[i])
                $display("pht[(%3h)%10b]: (%2b, %2b)",
                    i,
                    i,
                    touched[i][0] ? pht[i][0] : 2'bxx,
                    touched[i][1] ? pht[i][1] : 2'bxx
                );
        end
    ...
    //         o_pred[1]
    //     );
        $display("upd: {en: %b, take: %b, i_uhash: (%3h)%b, slot_idx: %b}", i_uen, i_utake, i_uhash, i_uhash, i_uslot_idx);
        $display("<< gshare");
    endtask
`endif
